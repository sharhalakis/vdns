...
def is_ttl(st):
    return st[0] in ('1', '2', '3', '4', '5', '6', '7', '8', '9', '0') and st[-1] != '.' and 'arpa' not in st
...
def parse_line(line0):
    ...
    for i in range(rridx):
        ...
        if ttl is None and is_ttl(items[i]):
            ttl = items[i]
        elif addr1 is None:
            addr1 = items[i]
        else:
    ...
    def read(self, fn, zone=None):
        ...
        for line0 in f:
            ...
            if r[3] == 'SOA':
                ...
                if r[0] != '@':
                    if r[0] != domain:
                        error("Domain doesn't match! (%s - %s)" % (domain, r[0]))
            ...
            if lastname is None and (r[0] is None or r[0] == '@'):
